                                    ...
                                    <div class="row">
                                        ...
                                        <div class="span6">
                                            <div class="dataTables_filter" id="example2_filter">
                                                <label style="line-height: 36px; display: block;">搜索用户组名: <input type="text" name="searchName" id="searchName" aria-controls="example2">
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered">
                                        ...
                                        <tbody id="datalist">
                                            ...
                                            <tr class="odd gradeX">
                                                ...
                                                <td class="center">
                                                    <a href="<?php echo Yii::$app->urlManager->createUrl(['iadmin/auth/assign', 'id' => $value['id']]) ?>">编辑权限</a>&nbsp;&nbsp;
                                                    <a href="<?php echo Yii::$app->urlManager->createUrl(['iadmin/auth/edit', 'id' => $value['id']]) ?>">编辑</a>&nbsp;&nbsp;
                                                    <a href="<?php echo Yii::$app->urlManager->createUrl(['iadmin/auth/delete', 'id' => $value['id']]) ?>">删除</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                        </tbody>
        ...
        $(function () {
            ...
            $('.pagination').children().find('a').on('click', function () {
                if(($(this).attr('data-page') + 1)) {
                    $.post('index.php?r=iadmin/auth/index', {'search_name': $('#searchNameHidden').val(), 'page': parseInt($(this).attr('data-page')) + 1}, function (jsonStr) {
            ...
            $('#searchBtn').on('click', function () {
                ...
                if($('#searchName').val()) {
                    $.post('index.php?r=iadmin/auth/index', {'searchName': $('#searchName').val(), 'pageSize': $('#pageSizeHidden').val()}, function (jsonStr) {
                        var jsonStr = $.parseJSON(jsonStr);
            ...
            $('#pageSize').on('change', function () {
                if($(this).val()) {  
                    $.post('index.php?r=iadmin/auth/index', {'searchName': $('#searchNameHidden').val(), 'pageSize': $(this).val(), 'page': $('.active').find('a').attr('data-page')}, function (jsonStr) {
                        var jsonStr = $.parseJSON(jsonStr);
